...
import { ALLOWED_MAJORS, ALLOWED_TOPIC_THESIS_TYPES } from '../config/validationConstants.js';
...
router.post('/generate', (req, res) => {
    ...
    if (!major || !ALLOWED_MAJORS.includes(major)) return res.status(400).json({ error: "Invalid major" });
    ...
    if (!thesis_type || !ALLOWED_TOPIC_THESIS_TYPES.includes(thesis_type)) return res.status(400).json({ error: "Invalid thesis type" });
