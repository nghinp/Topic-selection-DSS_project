...
if ([string]::IsNullOrWhiteSpace($ReportPath)) {
    $ReportPath = Join-Path $PSScriptRoot "temp-title-stress-report.json"
}

$qualityScriptPath = Join-Path $PSScriptRoot "temp-title-quality-test.ps1"
if (-not (Test-Path $qualityScriptPath)) {
    throw "Could not find quality test script at $qualityScriptPath"
}

$runReportDir = Join-Path $PSScriptRoot ".temp-title-stress-runs"
